<script>
	...
	async function updateFund() {
		try {
			...
			if (
				!newFund.year_of_occurance ||
				!newFund.applicant_country ||
				!newFund.name_of_disaster ||
				!newFund.disaster_type ||
				!newFund.status ||
				!newFund.first_damage_date ||
				!newFund.date_of_initial_application ||
				!newFund.time_from_disaster_to ||
				!newFund.major_regional_neighbouring ||
				!newFund.total_direct_damage_accepted ||
				!newFund.public_damage_eur_million ||
				!newFund.public_total_damage ||
				!newFund.cost_of_eligible_emergency ||
				!newFund.eligible_cost_total_damage ||
				!newFund.paid ||
				!newFund.advanced_payment_post_from_twenty_fifteen ||
				!newFund.potential_aid_amount_eur_m_
			) {
				message = 'Por favor, complete todos los campos'; // Muestra un mensaje de error
