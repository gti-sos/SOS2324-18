<script>
    ...
    onMount(() => {
        getFund(); // Llama a la función para obtener los datos del fondo
    });
    ...
    async function getFund() {
        try {
            ...
            message = "Error en el servidor"; // Muestra un mensaje de error si hay un problema con la solicitud
        }
    }

    // Función para actualizar los datos del fondo en la API
    async function updateFund(year, cci_number) {
        let response = await fetch(API + "/" +  cci_number, {
            method: "PUT",
            headers: {
  ...
  <Button on:click="{updateFund(newFund.year_of_occurance, newFund.cci_number)}" color="primary">Enviar</Button>
  <Button on:click={back} color="warning">Volver</Button>
  
  {/if}
  
  {#if message!=""}
  <!-- Mensaje emergente para mostrar el resultado de la operación -->
